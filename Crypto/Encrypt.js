...
function encrypt(data, passwd) {
    let hashedPasswd = hashPassword(passwd, nacl.secretbox.keyLength);
    let encryptionNonce = randomBytes(nonceLength());
    let encryptedData = nacl.secretbox(data, encryptionNonce, hashedPasswd.hash);

    return base32Encode(encryptionNonce) + '-' + base32Encode(hashedPasswd.salt) + '-' + base32Encode(encryptedData);
}
 ...
 * @returns {null}
 */
function decrypt(encryptedData, passwd, salt, nonce) {
    let hashedPasswd = hashPassword(passwd, nacl.secretbox.keyLength, salt);
    let data = null;
