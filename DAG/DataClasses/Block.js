...
class BlockBuilder {
    ...
    static Block(toAssign=undefined) {
        ...
        if (toAssign !== undefined)
            Object.assign(builder, toAssign);
        return builder;
    }
...
class Block {
    ...
    constructor(blockBuilder) {
        ...
        if (blockBuilder.sender === undefined)
            throw new Error("Blocks require a sender! set a sender when building with: .withSender(SENDER NAME HERE)");
        if (blockBuilder.recipient === undefined)
            throw new Error("Blocks require a recipient! set a recipient when building with: .withRecipient(RECIPIENT NAME HERE)");
        if (blockBuilder.amount === undefined)
            throw new Error("Blocks require an amount! set an amount when building with: .withAmount(AMOUNT HERE)");
        if (blockBuilder.previousHash === undefined)
            throw new Error("Blocks require a previousHash! set previousHash when building with: .withPreviousHash(PREVIOUS HASH HERE)");
    ...
    determineHash() {
        ...
        let amount = Crypto.encode(Crypto.hash(hexDecode(this.amount.toString(16))));
